...
proc countPaths(visitCave, targetCave: CaveNode, visited: var HashSet[string]): int =
  ...
  for cave in visitCave.neighbors:
    if cave.name notin visited:
      result += countPaths(cave, targetCave, visited)

  visited.excl visitCave.name
...
proc main() =
  ...
  echo "Paths: ", countPaths(startCave, endCave, visited)
